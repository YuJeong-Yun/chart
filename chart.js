...
function calcABDE() {
  ...
  if (a == "") {
    a =  (((1/(((100+d)/100)*((100+e)/100)*(1/I)))-((E/100)* (I/8 + 1/2)/100 *(1+4/I)*100+(E/100)* (I/8 + 1/2)/100 * (1+4/I)*((1+2/I)*100+(1+2/I)*(4/(3*I)*100)-100)+(F/100)* ((3/8)*I + 1/2)/100 *((1+2/I)*100+(1+2/I)*(4/(3*I)*100))+(H/100)* ((7/8)*I + 1/2)))/((E/100)* (I/8 + 1/2)/100 * (1+4/I)*(1+4/(3*I))+(F/100)* ((3/8)*I + 1/2)/100 * (1+4/(3*I))+(G/100)* ((5/8)*I + 1/2)/100 * (1+4/(3*I))+(H/100)* ((7/8)*I + 1/2)/100) );
    console.log("원래 a : "+a);
    // // 4주차 가격이 최대 가격보다 클 때
    ...
    d =  ((100/((E/100)* (I/8 + 1/2) * ((1/I)*(100+(1+4/I)*100+(1+4/I)*((1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)-100)/100) *((100+e)/100)+(F/100)* ((3/8)*I + 1/2) * ((1/I)*(100+(1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)/100) *((100+e)/100)+(G/100)* ((5/8)*I + 1/2) *((1/I)*(100+4/(3*I)*100+(1+4/(3*I))*a)/100)*((100+e)/100)+(H/100)* ((7/8)*I + 1/2) * ((1/I)*(100+a)/100) *((100+e)/100)))-100);
    console.log("원래 d : "+d);
    // // 4주차 가격이 최대 가격보다 클 때
    // if( (B*(H/100)*((A/I)* ((7/8)*I + 1/2) * (100+a)/100)) >= (A * B * (H/100)) ) {
    ...
    e = ((100/((E/100)* (I/8 + 1/2) * ((1/I)*(100+(1+4/I)*100+(1+4/I)*((1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)-100)/100) *((100+d)/100)+(F/100)* ((3/8)*I + 1/2) * ((1/I)*(100+(1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)/100) *((100+d)/100)+(G/100)* ((5/8)*I + 1/2) *((1/I)*(100+4/(3*I)*100+(1+4/(3*I))*a)/100)*((100+d)/100)+(H/100)* ((7/8)*I + 1/2) * ((1/I)*(100+a)/100) *((100+d)/100)))-100);
    console.log("원래 e : "+e);
    // // 4주차 가격이 최대 가격보다 클 때
    // if( (B*(H/100)*((A/I)* ((7/8)*I + 1/2) * (100+a)/100)) >= (A * B * (H/100)) ) {
...
function calcData(result) {
  ...
  for(let i =0; i < 5; i++) {
    w1 = (B*(E/100)* (I/8 + 1/2) * ((A/I)*(100+(1+4/I)*100+(1+4/I)*((1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*aRe-100)-100)/100) *((100+dRe)/100)*((100+eRe)/100)); // 1주차 가격
    w2 = (B*(F/100)* ((3/8)*I + 1/2) * ((A/I)*(100+(1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*aRe-100)/100) *((100+dRe)/100)*((100+eRe)/100)); // 2주차 가격
    w3 = (B*(G/100)* ((5/8)*I + 1/2) *((A/I)*(100+4/(3*I)*100+(1+4/(3*I))*aRe)/100)*((100+dRe)/100)*((100+eRe)/100)); // 3주차 가격
    w4 = (B*(H/100)* ((7/8)*I + 1/2) * ((A/I)*(100+aRe)/100) *((100+dRe)/100)*((100+eRe)/100)); // 4주차 가격
    // console.log(result+" - 수강료 대체  안한 원래  1,2,3,4주차 기대매출 : "+w1+", "+w2+", "+w3+", "+w4+" = 기대매출 합 "+(w1+w2+w3+w4));

    
    // 기대 매출 합계 계산
    let total = Math.floor(w1 + w2 + w3 + w4);
...
function calcAfterExpectSale() {
  ...
  let w1 = (B*(E/100)* (I/8 + 1/2) * ((A/I)*(100+(1+4/I)*100+(1+4/I)*((1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)-100)/100) *((100+d)/100)*((100+e)/100)); // 1주차 가격
  let w2 = (B*(F/100)* ((3/8)*I + 1/2) * ((A/I)*(100+(1+2/I)*100+(1+2/I)*(4/(3*I)*100)+(1+4/(3*I))*a-100)/100) *((100+d)/100)*((100+e)/100)); // 2주차 가격
  let w3 = (B*(G/100)* ((5/8)*I + 1/2) *((A/I)*(100+4/(3*I)*100+(1+4/(3*I))*a)/100)*((100+d)/100)*((100+e)/100)); // 3주차 가격
  let w4 = (B*(H/100)* ((7/8)*I + 1/2) * ((A/I)*(100+a)/100) *((100+d)/100)*((100+e)/100)); // 4주차 가격
...
function drawChartK() {
  ...
  // Add columns
  data.addColumn('number', 'k일 이용');
  data.addColumn('number', '금액');
  // Add empty rows
  data.addRows(kCostArr.length);
  for(let i=0; i<kCostArr.length; i++) {
    data.setCell(i, 0, i+1);
    data.setCell(i, 1, kCostArr[i]);
  }
